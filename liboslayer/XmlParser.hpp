...
namespace osl {
	...
	std::vector<AutoRef<XmlNode> > _children;
	std::string _text;
	
    public:
	XmlNode() : type(NONE), parent(NULL) {}
	virtual ~XmlNode() {
	    for (std::vector<AutoRef<XmlNode> >::iterator iter = _children.begin(); iter != _children.end();) {
		iter = _children.erase(iter);
	    }
	}
	void testType(int type) {
	    if (this->type != type) {
		throw Exception("type not matched / exptected: " + getTypeString(type) + ", but: " + getTypeString(this->type));
	    }
	}
	bool nil() {
	...
	AutoRef<XmlNode> child(size_t idx) {
	    testType(ELEMENT);
	    return _children[idx];
	}
	void addChild(AutoRef<XmlNode> node) {
	    testType(ELEMENT);
	    node->setParent(this);
	...
	AutoRef<XmlNode> getFirstChild() {
	    return *_children.begin();
	}
	AutoRef<XmlNode> getFirstChildElement() {
	    for (std::vector<AutoRef<XmlNode> >::iterator iter = _children.begin(); iter != _children.end(); iter++) {
		if ((*iter)->isElement()) {
		    return *iter;
		}
	    }
	    return AutoRef<XmlNode>();
	}
	AutoRef<XmlNode> getFirstChildText() {
	    for (std::vector<AutoRef<XmlNode> >::iterator iter = _children.begin(); iter != _children.end(); iter++) {
		if ((*iter)->isText()) {
		    return *iter;
		}
	...
	AutoRef<XmlNode> getLastChild() {
	    return *_children.rbegin();
	}
	std::vector<AutoRef<XmlNode> > & children() {
	    testType(ELEMENT);
	    return _children;
	}
	AutoRef<XmlNode> getElementByTagNameInDepth(const std::string & tagName, int depth) {
	    if (--depth <= 0) {
		return AutoRef<XmlNode>();
	    }
	    for (std::vector<AutoRef<XmlNode> >::iterator iter = _children.begin(); iter != _children.end(); iter++) {
		...
		AutoRef<XmlNode> elem = (*iter)->getElementByTagNameInDepth(tagName, depth);
		if (elem.nil() == false) {
		    return elem;
		}
	    }
	...
	AutoRef<XmlNode> getElementByTagName(const std::string & tagName) {
	    for (std::vector<AutoRef<XmlNode> >::iterator iter = _children.begin(); iter != _children.end(); iter++) {
		if ((*iter)->isElement() && (*iter)->tagName() == tagName) {
		    return *iter;
		}
		AutoRef<XmlNode> elem = (*iter)->getElementByTagName(tagName);
		if (elem.nil() == false) {
		    return elem;
		}
	    }
	...
	std::vector<AutoRef<XmlNode> > getElementsByTagNameInDepth(const std::string & tagName, int depth) {
	    std::vector<AutoRef<XmlNode> > lst;
	    if (depth-- <= 0) {
		return lst;
	    }
	    for (std::vector<AutoRef<XmlNode> >::iterator iter = _children.begin(); iter != _children.end(); iter++) {
		...
		std::vector<AutoRef<XmlNode> > ret = (*iter)->getElementsByTagNameInDepth(tagName, depth);
		if (ret.size() > 0) {
	...
	std::vector<AutoRef<XmlNode> > getElementsByTagName(const std::string & tagName) {
	    std::vector<AutoRef<XmlNode> > lst;
	    for (std::vector<AutoRef<XmlNode> >::iterator iter = _children.begin(); iter != _children.end(); iter++) {
		if ((*iter)->isElement() && (*iter)->tagName() == tagName) {
		    lst.push_back(*iter);
		}
		std::vector<AutoRef<XmlNode> > ret = (*iter)->getElementsByTagName(tagName);
		if (ret.size() > 0) {
		    lst.insert(lst.end(), ret.begin(), ret.end());
		}
	    }
	...
	static std::string toFullTagString(AutoRef<XmlNode> node) {
	...
	static std::string toTagNameString(AutoRef<XmlNode> node) {
	    if (!node->ns().empty()) {
		return node->ns() + ":" + node->tagName();
	    }
	    return node->tagName();
	}
	static std::string toString(AutoRef<XmlNode> node) {
	    std::string xml;
	    if (node.nil() == false) {
		if (node->isElement()) {
    ...
    class XmlNodeCursor {
    private:
	AutoRef<XmlNode> _root;
	XmlNode * _cursor;
    public:
	XmlNodeCursor() : _cursor(NULL) {}
	XmlNodeCursor(AutoRef<XmlNode> root) : _root(root), _cursor(&root) {}
	virtual ~XmlNodeCursor() {}
	void enter(AutoRef<XmlNode> element) {
	...
	void append(AutoRef<XmlNode> node) {
	    _cursor->addChild(node);
	}
	XmlNode * cursor() {
	    return _cursor;
	}
	AutoRef<XmlNode> root() {
	    return _root;
	}
    };
	...
	static void parseAttrs(const std::string & attrs, AutoRef<XmlNode> node) {
	    if (attrs.empty()) {
		return;
	    }
	...
	static void parseTag(const std::string & tag, AutoRef<XmlNode> node) {
	    std::string ns;
	    std::string name;
	    std::string attrs;
	...
	static XmlDocument parse(const std::string & text) {
		...
		if (e == std::string::npos) {
		    throw Exception("Wrong xml format");
		}
				
		if (text[e - 1] == '/') {
		    // atom
		    AutoRef<XmlNode> node(new XmlNode);
	    ...
	    doc.rootNode() = AutoRef<XmlNode>(cursor.root());
	    return doc;
	}
    };
}
